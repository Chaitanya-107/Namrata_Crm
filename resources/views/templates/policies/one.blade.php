...
@section('profile_bar')
    ...
    <div class="ui segment white right-bar-profile right-bar-profile-bottom">
        ...
        @@if (!empty($policy->client) && $policy->client->profile_image_filename === 'default-profile.jpg')
...
@elseif (!empty($policy->client) && !empty($policy->client->profile_image_filename))
                ...
                <tbody>
                    @forelse($policy->attachments as $attachment)
                        <tr>
                            <td>
                            @php
    $extension = pathinfo(storage_path('app/attachments/' . $attachment->filename), PATHINFO_EXTENSION);
    $iconClass = array_key_exists($extension, [
        'bmp'   => 'image',
        'doc'   => 'word',
        'docx'  => 'word',
        'gif'   => 'image',
...
@endphp
                                ...
                                <i class="{{ $iconClass }} outline icon"></i> {{ $attachment->name }}
                            </td>
