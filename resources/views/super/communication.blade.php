            ...
            <form action="{{ action('EmailController@add') }}" method="POST">
                ...
                <div class="ui form">
                    <div class="field required">
                        ...
                        <select name="recipient" class="ui fluid search dropdown">
                            <option value="">{{ trans('communication.input.option.default') }}</option>
                            @if ($admins->count() > 0)
                                <option{{ old('recipient') === 'admins' ? ' selected' : '' }} value="admins">{{ trans('communication.input.option.admins') }}</option>
                            @endif
                            @if ($user->company->brokers->count() > 0)
                                <option{{ old('recipient') === 'brokers' ? ' selected' : '' }} value="brokers">{{ trans('communication.input.option.brokers') }}</option>
                            @endif
                            @if ($user->company->clients->count() > 0)
                            ...
                            @if ($user->company->staff->count() > 0)
                                <option{{ old('recipient') === 'staff' ? ' selected' : '' }} value="staff">{{ trans('communication.input.option.staff') }}</option>
                            @endif
                            @foreach ($user->company->users->except($user->id) as $company_user)
