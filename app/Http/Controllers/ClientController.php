...
class ClientController extends Controller
{
    ...
    {
        // Assuming you want to fetch all clients and pass them to the view
        $clients = Client::all();
    ...
    |--------------------------------------------------------------------------
    |
    | This controller handles the management and update of existing clients.
    |
    */
    ...
    public function add(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'first_name'        => 'max:32|min:3|string|required|alpha',
            'inviter_id'        => 'exists:users,id|integer|sometimes',
            'last_name'         => 'max:32|min:3|string|alpha',
            'phone'             => 'max:16|string',
            'profile_image'     => 'image',
            'password'          => 'required|min:8|confirmed', // Ensure password and password_confirmation fields are present and match
        ]);
...
$client->save();
    ...
    public function edit(Request $request, User $client)
    {
        $validator = Validator::make($request->all(), [
            ...
            'first_name'    => 'max:32|min:3|string|required|alpha', // Added alpha rule here
            'inviter'       => 'exists:users,id|integer|sometimes',
            'last_name'     => 'max:32|min:3|string|alpha', // Added alpha rule here
            'phone'         => 'max:16|string',
            'profile_image' => 'image'
        ]);
    ...
    public function getAll(Request $request)
    {
        ...
        switch ($user->role) {
            case 'super':
                ...
                $view_data['clients'] = User::client()->withStatus()->insuraFilter($filters)->orderBy('id', 'desc')->simplePaginate(8);
            ...
            case 'admin':
            case 'staff':
                $view_data['clients'] = $user->company->clients()->insuraFilter($filters)->orderBy('id', 'desc')->withStatus()->simplePaginate(8);
                $view_data['clients']->currency_symbol = $currencies_by_code->get($user->company->currency_code)['symbol'];
                break;
            case 'broker':
                $view_data['clients'] = $user->invitees()->insuraFilter($filters)->orderBy('id', 'desc')->withStatus()->simplePaginate(8);
                $view_data['clients']->currency_symbol = $currencies_by_code->get($user->company->currency_code)['symbol'];
                break;
        }
